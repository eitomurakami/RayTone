...
sealed class MidiOutTest : MonoBehaviour
{
    ...
    System.Collections.IEnumerator Start()
    {
        ...
        for (var i = 0; true; i++)
        {
            ...
            foreach (var port in _ports) port?.SendControlChange(10, 25, note);

            Debug.Log("MIDI Out: Note On " + note);
            foreach (var port in _ports) port?.SendNoteOn(10, note, 100);

            yield return new WaitForSeconds(0.1f);

            Debug.Log("MIDI Out: Note Off " + note);
            foreach (var port in _ports) port?.SendNoteOff(10, note);
