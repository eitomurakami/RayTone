...
public class ChuckSubInstance : MonoBehaviour
{
    ...
    private string myOutputUgen = "__dac__";
    ...
    void Awake()    //EM Changed from Awake() to Start() to allow overwriting chuckMainInstance at init
    {
    ...
    public void Init()
    {
        ...
        if (isVoice)
        {
            ...
            myOutputUgen = myOutputUgen + GetID();
    ...
    void OnAudioFilterRead( float[] data, int channels )
    {
        ...
        // EM - check if data size is correct
        if (myMonoBuffer.Length != numFrames)
        {
            myMonoBuffer = new float[numFrames];
        }
