...
public class ChuckMainInstance : MonoBehaviour
{
    ...
    // EM - microphone is set up in Start to account for permission request to take place.
    private void Start()
    {
        // setup mic
        if (useMicrophone)
    ...
    private void SetupMic()
    {
        ...
        {
            RayTone.Console.Log("No microphone device detected.\n");
            return;
        }
        ...
        //Check if the microphone clip was initialized properly
        if (micClip == null)
        {
            return;
        }
        ...
        // play audio source!
        mySource.Play();
        Invoke(nameof(ApplyMicrophoneLatencyOffset), 1f);
#endif
    }

    /// <summary>
    /// EM - move playhead to (current microphone position - audio buffer size) to account for potential latency. 
    /// </summary>
    private void ApplyMicrophoneLatencyOffset()
    {
        float originalTime = mySource.time;
        mySource.time = (Microphone.GetPosition("") - AudioSettings.GetConfiguration().dspBufferSize * 1.0001f) / (float)AudioSettings.GetConfiguration().sampleRate;
        RayTone.Console.Log("Applied " + (mySource.time - originalTime) * 1000f + " ms microphone latency offset.");
    }
    ...
    {
        // EM - check whether data size is correct
        if( (channels != myNumChannels) || (data.Length != myOutBuffer.Length) )
        {
            // sadness -- data size has changed so we must reconstruct myOutBuffer
    ...
    /// <summary>
    /// EM - Called by RayTone.UnitController if audio buffer size changes
    /// </summary>
